...
const Mutations = {
    ...
    return ctx.db.mutation.deleteItem({ where }, info);
      ...
      async signup(parent, args, ctx, info){
        ...
        const pwd=await bcrybt.hash(args.password,10)
        const user=await ctx.db.mutation.createUser({data:{
          ...args,password:pwd,permissions:{set:['USER']}
        }},info)
        //create JWT token
      ...
      async signin(parent, {password,email}, ctx, info){
        ...
        if(!user){
          throw new Error(`No such email: ${email} found`)
        }
